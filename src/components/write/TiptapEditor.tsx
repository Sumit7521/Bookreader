...
import { Save, Bold, Italic, Strikethrough, Heading1, Heading2, List, ListOrdered, Quote, Undo, Redo, ChevronLeft } from 'lucide-react'
import { Button } from '@/components/ui/button'
import { saveChapterContentAction } from '@/actions/write'
import Link from 'next/link'

type ChapterType = { _id: string, title: string, content?: string };
export function TiptapEditor({ chapter, projectId }: { chapter: ChapterType, projectId: string }) {
    ...
    <div className="flex flex-col h-[calc(100vh-6rem)] animate-in fade-in duration-500">
      <div className="flex items-center justify-between mb-4 border-b border-stone-200 dark:border-stone-800 pb-4">
        <div className="flex items-center gap-4">
          ...
          <h2 className="text-xl font-serif font-bold text-stone-800 dark:text-stone-200">{chapter.title}</h2>
        </div>
        <div className="flex items-center gap-4">
      ...
      <div className="flex-1 overflow-y-auto pb-20 scrollbar-thin scrollbar-thumb-stone-300 dark:scrollbar-thumb-stone-700">
        <div className="max-w-3xl mx-auto py-12 px-8 sm:px-12 bg-white dark:bg-[#1a1a1a] shadow-sm border border-stone-200 dark:border-stone-800 min-h-full rounded-sm">
